...
static void processMinf(ISOBMFF::Container* minf) {
  ...
  if (!stblbox) {
    ...
    if (!stbl) {
      ...
      std::shared_ptr<ISOBMFF::STSC> stsc =
          stbl->GetTypedBox<ISOBMFF::STSC>("stsc");
      if (!stsc) {
        ...
        for (size_t i = 0; i < stsc->GetEntryCount(); i++) {
          ...
          std::cout << "idx  : " << i << " sampleToChunk.firstChunk : "
                    << sampleToChunk.firstChunk
                    << " sampleToChunk.samplesPerChunk : "
                    << sampleToChunk.samplesPerChunk
                    << " sampleToChunk.sampleDescriptionId : "
                    << sampleToChunk.sampleDescriptionId << "\n";
        }
      }
      ...
      std::shared_ptr<ISOBMFF::STCO> stco =
          stbl->GetTypedBox<ISOBMFF::STCO>("stco");
      if (!stco) {
      ...
      std::shared_ptr<ISOBMFF::CO64> co64 =
          stbl->GetTypedBox<ISOBMFF::CO64>("co64");
      if (!co64) {
...
static void processTrak(ISOBMFF::Container* trak) {
  ...
  if (!mdiabox) {
    ...
    if (!minfbox) {
      ...
      ISOBMFF::Container* minf =
          dynamic_cast<ISOBMFF::Container*>(minfbox.get());
      if (minf) {
        processMinf(minf);
      } else {
...
bool parseFile(std::string filename) {
  ...
  try {
    ...
    std::shared_ptr<ISOBMFF::FTYP> ftyp =
        file->GetTypedBox<ISOBMFF::FTYP>("ftyp");
    if (!ftyp) {
    ...
    std::cout << "mvhd duration : " << mvhd->GetDuration()
              << " scale : " << mvhd->GetTimescale() << "\n";

    std::vector<std::shared_ptr<ISOBMFF::Box> > tracks =
        moov->ISOBMFF::Container::GetBoxes("trak");
